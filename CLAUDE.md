...
All 120 migrations replay on every remote deploy because the `__drizzle_migrations` ledger comes back empty. **This is still open, and it has two layers.** The first is fixed: the ledger SELECT reached wrangler split into four argv tokens (`Unknown arguments: hash, FROM, __drizzle_migrations;`) and is now written space-free — see the comment above `LEDGER_SELECT` in `packages/db/src/sqlite/migrate-d1.ts`. That error is gone from the build log, and it exposed the second layer: the writes themselves exit non-zero (`wrangler exit ≠ 0 — likely partial replay`), so no hash is ever recorded. That message used to swallow wrangler's stderr, which is why the real failure stayed invisible for months; both the migration apply and the ledger INSERT now print wrangler's own words. Read those lines in the next build log before theorising.
