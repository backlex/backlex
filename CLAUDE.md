...
**Almost none of a CF build is compilation — it is the D1 migration step.** Measured on a green 16.0-minute build: migrations ran 0.5m → 15.5m, then the whole `vite build` took **12 seconds** (`backlex_admin` env 2.37 s + client env 9.02 s). The cause was `migrate-d1.ts` being unable to read its `__drizzle_migrations` ledger on the runner, so all 120 migrations replayed on every single deploy — see the comment above `LEDGER_SELECT` in `packages/db/src/sqlite/migrate-d1.ts` for why the query is written without spaces. If a build ever creeps back toward the runner's 31-minute kill, read the migration timestamps first; the bundler is not where the minutes go.
...
- **`vite` is pinned to exactly `8.0.13` on purpose — do not bump it casually.** Vite 8 bundles with rolldown, and rolldown ≥ 1.0.3 deadlocks on CPU-constrained CI when a JS plugin's `transform` hook calls a native NAPI addon — for us `@tailwindcss/vite` → `@tailwindcss/oxide` (rolldown [#10102](https://github.com/rolldown/rolldown/issues/10102), closed `not_planned`; [#10664](https://github.com/rolldown/rolldown/issues/10664)). It builds fine locally on a many-core machine and then hangs forever on the Cloudflare Builds runner, killed at its 31-minute limit with no error — we shipped `8.2.1` (rolldown 1.2.4) and lost two deploys that way. `8.0.13` resolves rolldown `1.0.1` and has an unbroken record here. The upstream env-var workarounds (`TOKIO_WORKER_THREADS`, `RAYON_NUM_THREADS`, `UV_THREADPOOL_SIZE`) were all tried and none help.

  **How strong the evidence actually is (measured 2026-08-15, read this before acting on the pin).** The vite bump to `8.2.1` and the move to `bunx --bun vite build` landed in the *same* push, the hangs started after it, and the pin reverted only the first — so the two are confounded and **`8.2.1` on plain Node was never tested**. `8.2.1` also produced one green 19.1-minute build, so the failure is a race at roughly 2-in-3, and a single green build on `8.0.13` had about a 1-in-3 chance of being luck. The hang itself is real, not slowness: 16.5 minutes of silence at a stage that takes 9 seconds in a green build. Treat the pin as a working precaution, not a proven diagnosis.

  **The pin needs a `resolutions` entry to bind transitive copies.** `apps/site/package.json` names `vite` directly, yet astro pulled its own nested `vite@8.2.1` (→ rolldown `1.2.4`) straight past it, because a direct dependency pin does not constrain a dependency's own dependency. Root `resolutions` now carries `"@backlex/site/astro/vite": "8.0.13"`. It is scoped on purpose — `apps/docs` runs astro 6, which requires `vite ^7.3.2`, so a blanket `"vite"` resolution would force a major mismatch on it. Verify with `grep -oE '"[^"]*vite": \["vite@[0-9.]+' bun.lock`, never with package.json. **Known cost:** `8.0.13` carries two open advisories — GHSA-fx2h-pf6j-xcff (high, `server.fs.deny` bypass) and GHSA-v6wh-96g9-6wx3 (medium, launch-editor NTLM disclosure), both fixed in `8.0.16`. Both are Windows-only and dev-server-only, and vite never ships to production, so the exposure here is nil while the deadlock is not. Move to `8.0.16`+ once rolldown fixes the deadlock — verify by watching a real Cloudflare build to completion, not a local build.
