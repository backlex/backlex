...
/**
 * Internal endpoint used **only** by the cf-dispatch executor sub-Worker to
 * proxy `ctx.fetch / ctx.db / ctx.email` calls back into the main Worker.
 *
 * Authentication: shared-secret bearer token (`env.SANDBOX_RPC_TOKEN`). The
 * token is generated by the operator and configured on both the main Worker
 * (env) and the executor (the main Worker forwards it in the `/run` body so
 * the sub-Worker can include it in its callbacks).
 *
