 ...
 *     `apps/web/tests/consent/consent-banner.test.ts`". The migration bundles had no
...
describe("migrations manifest ↔ shipped migrations", () => {
  for (const { dialect, bundle, tags } of DIALECTS) {
    test(`${dialect}: every shipped migration's sha256 resolves to its folder tag`, () => {
      ...
      // packages/db/scripts/build-manifest.ts and has NO lefthook job, so this
      // test is the only thing keeping it in step with the bundle.
      //
      // Staleness USED to be cosmetic (the admin Migrations page rendered
      // `tag: null` for an unmapped hash) and this comment used to say so. It
      // is not any more: `auto-migrate.ts::adoptCliLedger` maps a CLI ledger's
      // hashes through this manifest to decide which migrations a database has
      // already had. A hash missing from here is a migration the runner cannot
      // recognise as applied — so it replays it, and for the six files that
      // transform data that is the phase-8 defect coming back. See
      // `security-audit-2026-09-migration-replay.test.ts`.
      const wrong: string[] = [];
      for (const migration of bundle) {
