 ...
 * requires `{model, text, topK?}`, so every generated call hard-failed on the
 * Zod schema with "model invalid" + "text undefined".
 *
 * This test pins the actual route contract so the planner description in
 * ai-ask.ts can't silently drift back: the OLD shape must fail validation
 * (422, reproducing the user-reported bug), and the NEW shape must clear
 * validation (it may still fail downstream on the missing Vectorize binding —
 * that's infra, not the request contract, so we assert it is NOT a
 * validation error).
 */
...
describe("vector.search request contract", () => {
  ...
  test("OLD planner shape {collection, query, top_k} → 422 on model + text", async () => {
  ...
  test("NEW planner shape {model, text, topK} → clears validation", async () => {
    const res = await search({
      model: "bge-m3",
    ...
    // but it must get PAST request validation. A 422 here would mean the
