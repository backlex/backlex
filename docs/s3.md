...
`?uploadId=` and `?prefix=` are the only caller-supplied fragments this endpoint
...
- **`uploadId`** is opaque here — it was minted by whichever backend is
  configured and is handed straight back to it — so the check is a deny list of
  what no backend ever issues: a separator, a parent-directory hop, a NUL, a
  control character, or more than 1 024 characters. Anything else is
  `InvalidArgument` (400). The fs adapter applies a stricter allow list of its
  own before the value becomes part of a filename, and re-checks the finished
  path against the storage root; a server-issued id (a UUID there, base64url-ish
  on R2 and S3) is unaffected, including one already in flight.
- **`prefix`** is now judged by the same rule as a key minus the parts that only
  make sense for a whole filename — it may be empty, and its last segment is a
  partial name, so `report.` still lists the reports. A traversal prefix is
  `InvalidArgument` (400); previously it reached the adapter and surfaced as an
  unhandled 500.
