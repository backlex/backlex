...
import { Badge, Card, EmptyState, PageHeader, Skeleton, Table, TableScroll, Td, Th, cx, inputCls } from "../lib/ui";
      ...
      <div className="mt-4 grid gap-4 lg:grid-cols-3">
        <Card className="lg:col-span-2">
          <div className="mb-3 flex items-center justify-between">
            ...
            <a href="#/orders" className="text-sm text-indigo-300 hover:underline">
              All orders →
            </a>
          </div>
          {recent.data == null ? (
                  ...
                  {recent.data.map((o) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <Td className="max-w-[16ch] truncate text-white/70">{who || "—"}</Td>
                        <Td>
        ...
        <div className="space-y-4">
          <Card>
            ...
            {catalog.data == null ? (
              ...
              <dl className="grid grid-cols-2 gap-3">
                <div>
                  <dt className="text-xs text-white/45">Active</dt>
                  <dd className="text-xl font-semibold tabular-nums">{fmtNumber(catalog.data.active)}</dd>
                </div>
                <div>
                  <dt className="text-xs text-white/45">Draft</dt>
          ...
          <Card>
            ...
            {lowStock.data == null ? (
              ...
              <p className="text-sm text-white/45">Every stocked line is above its reorder point.</p>
            ) : (
                  ...
                  return (
                    <li key={l.id} className="flex items-center justify-between gap-2">
                      <span className="min-w-0 truncate">
                        ...
                        <span className="text-white/40"> · {(loc && typeof loc === "object" && loc.name) || "—"}</span>
                      </span>
...
function KpiTile({ kpi }: { kpi: KpiResult }) {
  ...
  return (
    <Card>
      <p className="truncate text-xs text-white/45" title={kpi.description ?? kpi.name}>
        {kpi.name}
      </p>
      {rows.length === 0 ? (
...
function goodness(direction: KpiResult["direction"], pct: number): string {
  if (direction === "neutral") return "text-white/50";
