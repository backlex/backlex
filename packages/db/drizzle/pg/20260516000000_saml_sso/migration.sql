...
-- audit trail. See packages/db/src/pg/schema.ts:samlProviders /
...
CREATE UNIQUE INDEX IF NOT EXISTS "saml_providers_tenant_slug_idx" ON "saml_providers" ("tenant_id","slug");
--> statement-breakpoint
...
--> statement-breakpoint
CREATE UNIQUE INDEX IF NOT EXISTS "external_identities_lookup_idx" ON "external_identities" ("tenant_id","provider_type","provider_id","subject");
--> statement-breakpoint
CREATE INDEX IF NOT EXISTS "external_identities_user_idx" ON "external_identities" ("plane","user_id");
